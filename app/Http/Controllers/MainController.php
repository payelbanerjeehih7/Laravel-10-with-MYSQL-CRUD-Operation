...
class MainController extends Controller
{
    ...
    {
        $userid=$dc;
        $user=DB::table('finalassessments')->where('id','=',$userid)->get();
    ...
    public function update(Request $request)
    {
        ...
        DB::table('finalassessments')->where('id','=',$userid)->update($data);
        $user=DB::table('finalassessments')->where('id','=',$userid)->get();
        return view('displayclient')->with(['clientinfo'=>$user]);
        }
        else
    ...
    public function changepasswordaction(Request $request)
    {
        ...
        $data=DB::table('finalassessments')->where('id','=',$userid)->get();
        ...
        else
        {
            if($oldp!=$newp)
            {
                if($newp==$confp)
                {
                    DB::table('finalassessments')->where('id','=',$userid)->update(['password'=>$newp]);
    ...
    {
        $userid=$blk;
        DB::table('finalassessments')->where('id','=',$userid)->update(['auth'=>1]);
        return redirect('/displayall')->with('message','User has been blocked');
    }
    public function unblock($ublk)
    {
        $userid=$ublk;
        DB::table('finalassessments')->where('id','=',$userid)->update(['auth'=>0]);
        return redirect('/displayall')->with('message','User has been unblocked');
    }
    public function delete($del)
    {
        $userid=$del;
        DB::table('finalassessments')->where('id','=',$userid)->delete();
